<template>
  <div>
    ...
    <div v-else>Use the bottom right pen button to add a node.</div>

    <b-card-group columns>
      <!-- <b-row>
      <b-col v-for="(node, i) in orderedNodes" :key="i"> -->
      <NodeComponent
        v-for="(node, i) in orderedNodes"
        :key="i"
        :node="node"
        @delete="deleteNode"
        @edit="editNode"
    ...
    <div v-if="notBoth.length > 0">
      <h2>not both</h2>
      <b-table small responsive striped hover :items="notBoth" :fields="fields">
        <template #cell(id)="data">
          {{
            (data.item.local && data.item.local["ve:name"]) ||
            (data.item.remote && data.item.remote["ve:name"])
          }}
          <hr />
          {{ data.item.id }}
        </template>
        <template #cell(update)="data">
          ...
          <b-button-toolbar>
            <b-button-group class="mx-1">
              <b-button
                v-if="data.item.local == null"
                @click="createLocal(data.item.remote)"
                variant="success"
                >&laquo;</b-button
              >
              <b-button
                v-if="data.item.remote == null"
                @click="removeLocal(data.item.local)"
                variant="danger"
                >X</b-button
              >
              <!-- </b-button-group>
            ...
            <b-button-group class="mx-1"> -->
              <b-button
                v-if="data.item.local == null"
                @click="removeRemote(data.item.remote)"
                variant="danger"
                >X</b-button
              >
              <b-button
                v-if="data.item.remote == null"
                @click="createRemote(data.item.local)"
                variant="success"
                >&raquo;</b-button
              >
            </b-button-group>
    ...
    <div v-if="mustUpdate.length > 0">
      ...
      <b-table striped small responsive hover :items="mustUpdate" :fields="fields2">
        ...
        {{data.item}} -->
          <b-button-toolbar>
            <b-button-group class="mx-1">
              <b-button @click="updateLocal(data.item.remote)"
                >&laquo; update local</b-button
              >
              <b-button @click="updateRemote(data.item.local)"
                >update remote &raquo;</b-button
              >
            </b-button-group>
          </b-button-toolbar>
          <!-- {{data}} -->
        ...
        <template #cell(difference)="data">
          <small>
            <ul>
              <li v-for="(diff, key) in data.item.difference" :key="key">
                {{ key }} :<br />
