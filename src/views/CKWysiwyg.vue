  ...
  <div>
    <div v-if="storage != false">
      ...
      <b-breadcrumb>
        ...
        <b-breadcrumb-item v-for="p in path" :key="p.text" @click="changePath(p)">{{
          p.text
        }}</b-breadcrumb-item>
        ...
        <div v-if="addingFile">
          ...
          <b-button variant="primary" size="sm" @click="addingFile = false"
            >cancel</b-button
          >
        </div>
        <div v-else>
      ...
      <div v-if="selected != null">
        Share your
        <b
          ><a :href="selected.url" target="_blank">{{ selected.text }}</a></b
        >
        page.
      </div>
      <ckeditor
    ...
    <div v-else>Please login to update files on your pod</div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    if (this.storage != false) {
      this.path = [{ text: "Home", url: this.storage, type: "folder" }];
