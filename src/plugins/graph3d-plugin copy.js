...
const plugin = {
  install(Vue, opts = {}) {
    ...
    function getSize() {
      ...
      return {
        w: window.innerWidth > 768 ? window.innerWidth / 2.05 : window.innerWidth * 0.99,
        h: window.innerWidth > 768 ? window.innerHeight * 0.9 : window.innerHeight * 0.65
      }
    }
    ...
    Vue.prototype.$zoomToFit = function (/*params*/) {
      ...
      store.state.core.graph.zoomToFit(10, 10, (node) => {
        store.state.core.search == null ||
          (store.state.core.search.text.length > 0 &&
            node.name.includes(store.state.core.search.text))
