...
export class LlamaCppEmbeddings extends Embeddings {
    constructor(inputs) {
        ...
        this._inputs = inputs;
        this._inputs.embedding = true;
      }

    async init() {
        let _inputs = this._inputs
        this._model = await createLlamaModel(_inputs);
        this._context = await createLlamaContext(this._model, _inputs);

        return {model: this._model, context: this._context}
    }
    /**
